...
def draw_example(ch, dst_font, canvas_size, x_offset, y_offset, filter_hashes):
...
def font2img(dst, charset, char_size, canvas_size,
    ...
    for c in charset:
        if count == sample_count:
            break
        e = draw_example(c, dst_font, canvas_size, x_offset, y_offset, filter_hashes)
        if e:
            e.save(os.path.join(sample_dir, "%d_%04d.jpeg" % (label, count)))
            count += 1
            if count % 100 == 0:
